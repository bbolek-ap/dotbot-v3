...
- `research-repos.md` must exist — this may be a **seed file** from Phase 01 (extracted from briefing docs) or a full Sourcebot scan from Phase 02c. Parse identically regardless of origin.
...
> **Note:** `research-repos.md` uses the same tier-table format whether it was seeded from briefing docs or generated by Sourcebot. Seed files may have fewer tiers populated and `TBD` for some Project values — handle these gracefully (skip rows with `TBD` project if repo cloning requires a valid project).
