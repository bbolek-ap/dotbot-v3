...
function Write-ActivityLog {
    ...
    # Delegate to unified structured log (Write-BotLog writes to dotbot-{date}.jsonl)
    try {
        Write-BotLog -Level Info -Message $sanitizedMessage -Context @{ type = $Type; phase = $effectivePhase }
    } catch {}
}
...
function Invoke-ClaudeStream {
    ...
    try {
        ...
        try { Write-BotLog -Level Fatal -Message "Failed to start claude process: $($_.Exception.Message) (args length: $argLen)" -Context @{ source = 'claude-cli'; error_code = 'PROCESS_START_FAILED' } -Exception $_ } catch {}
        throw
    }
    ...
    $processLine = {
            ...
            if ($line[0] -eq '{' -and $line -match "hit your limit|error.*rate_limit") {
                try {
                    ...
                    if ($rateLimitText) {
                        ...
                        Write-BotLog -Level Warn -Message "Rate limit hit: $rateLimitText" -Context @{ source = 'claude-cli'; error_code = 'RATE_LIMIT' }
                        return
                    }
                } catch {
        ...
        if ($evt.type -eq "result") {
            ...
            Write-BotLog -Level Warn -Message "Stream event processing error: $($_.Exception.Message)" -Context @{ source = 'claude-cli' } -Exception $_
        }
    }
    ...
    while ($true) {
        ...
        try {
            ...
            Write-Debug "Error processing stream event: $($_.Exception.Message)"
            Write-BotLog -Level Warn -Message "Stream line processing error: $($_.Exception.Message)" -Context @{ source = 'claude-cli' } -Exception $_
        }
    }
