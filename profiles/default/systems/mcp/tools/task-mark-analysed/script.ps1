...
function Invoke-TaskMarkAnalysed {
    ...
    # Build analysis object with timestamp
    $analysisWithTimestamp = $analysis.Clone()
    $analysisWithTimestamp['analysed_at'] = $now
    ...
    # Close current Claude session (post-move in-place update)
    ...
    return @{
        ...
        new_status            = 'analysed'
        analysis_completed_at = $result.task.analysis_completed_at
